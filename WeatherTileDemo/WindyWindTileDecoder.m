...
+ (Header)decodeHeader:(uint32_t *)pixels width:(NSInteger)width {
    ...
    // DEBUG: 打印打包后的字节
    NSLog(@"[DECODER] Packed bytes (28 bytes for 7 floats):");
    for (int i = 0; i < 28; i++) {
        if (i % 4 == 0) NSLog(@"[DECODER]   Float %d: %02X %02X %02X %02X", i/4, bytes[i], bytes[i+1], bytes[i+2], bytes[i+3]);
    }
    NSLog(@"[DECODER] All 28 header pixel RGB values:");
    for (int i = 0; i < 28; i++) {
        uint32_t p = pixels[pixelIndex + i * 4];
        uint8_t r = (p >> 16) & 0xFF;
        uint8_t g = (p >> 8) & 0xFF;
        uint8_t b = p & 0xFF;
        int rp = (int)roundf(r / 64.0f) & 0x3;
        int gp = (int)roundf(g / 16.0f) & 0xF;
        int bp = (int)roundf(b / 64.0f) & 0x3;
        uint8_t packed = (rp << 6) | (gp << 2) | bp;
        NSLog(@"[DECODER]   Pixel[%d]: RGB(%d,%d,%d) -> packed bits: r=%d g=%d b=%d -> byte=0x%02X", 
              i, r, g, b, rp, gp, bp, packed);
    }
